...
| [codex-delegate](skills/codex-delegate) | Drive the OpenAI Codex CLI as a sub-agent for a typed task — review, implement, diagnose, or research — through a thin wrapper over `codex exec` that picks the right sandbox and invocation per type and returns a clean result. |
